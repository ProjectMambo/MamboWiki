...
- A keyboard-first launcher provides Apps, Run, Windows, Power, and binary-safe Clipboard modes, follows the focused monitor while dimming every output, accepts validated AGS requests, switches modes with `Alt-Shift-1` through `Alt-Shift-5`, exposes complete app and 5,000-entry Cliphist result sets through a virtualized scrolling list, supports dedicated-GPU application launch, focuses mapped Hyprland clients, executes commands without a shell, and confirms disruptive power actions.
...
13. **Deep launcher lists — implemented:** show the complete desktop-application list immediately, search every retained clipboard entry, virtualize the shared scrolling result view, and raise native Cliphist retention to 5,000 entries without adding a second history store.
