...
- The left overlay shows battery status and care, CPU/iGPU telemetry, fan RPM, firmware thermal profiles, confirmed graphics-mode requests, and current display brightness. Raw fan curves and CPU-governor switches stay out of AGS. Brightness changes remain disabled until a narrow FA507XV host permission is applied.
...
The remaining shell design is planned: extend the main menu with commands, windows, and power actions, replace Mako only when AGS owns both notification popups and history, add the device-specific brightness permission, and validate recovery paths before changing session startup or keybindings.
